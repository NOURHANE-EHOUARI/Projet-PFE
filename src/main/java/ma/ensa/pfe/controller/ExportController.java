...
public class ExportController {
    ...
    public String versions(Model model) {
        ...
        List<Map<String, String>> versions = new java.util.ArrayList<>();

        if (base.exists()) {
            java.io.File[] dossiers = base.listFiles(f -> f.isDirectory() && f.getName().startsWith("version_"));
            if (dossiers != null) {
                java.util.Arrays.sort(dossiers, Comparator.comparingLong(java.io.File::lastModified).reversed());
                for (java.io.File d : dossiers) {
                    ...
                    long size = java.util.Arrays.stream(d.listFiles() != null ? d.listFiles() : new java.io.File[0])
                        .mapToLong(java.io.File::length).sum();
                    v.put("taille", (size / 1024) + " Ko");
                    v.put("fichiers", String.valueOf(d.listFiles() != null ? d.listFiles().length : 0));
                    versions.add(v);
                }
            }
        }
